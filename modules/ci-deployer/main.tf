...
resource "google_service_account_iam_member" "act_as_front_runtime" {
  service_account_id = "projects/${var.gcp_project_id}/serviceAccounts/${var.front_runtime_service_account_email}"
  role               = "roles/iam.serviceAccountUser"
  member             = "serviceAccount:${google_service_account.github_deploy.email}"
}
